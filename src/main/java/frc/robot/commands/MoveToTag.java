...
public class MoveToTag extends Command {
  ...
  private static final TrapezoidProfile.Constraints OMEGA_CONSTRAINTS =   new TrapezoidProfile.Constraints(1, 8);
  ...
  private final int TAG_TO_CHASE;
  ...
  public MoveToTag(
        PhotonPipelineResult camResult, 
        DriveSubsystem drivetrainSubsystem,
        Supplier<Pose2d> poseProvider,
        Supplier<Integer> tag) {
    this.TAG_TO_CHASE = tag.get();
    this.photonRes = camResult;
  ...
  public void execute() {
    ...
    if (photonRes.hasTargets()) {
      ...
      if (targetOpt.isPresent()) {
        ...
        var cameraPose = robotPose.transformBy(Constants.VisionConstants.kRobotToCam);
