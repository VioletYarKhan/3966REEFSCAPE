...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_coralHand.setDefaultCommand(
      new RunCommand(
        ()-> {
            if(m_wrist.getVelocity() > 600 || (currentState != State.L4 && currentState != State.Intake && !m_coralHand.hasCoral())){
              m_coralHand.intake();
            } else {
  ...
  private void configureButtonBindings() {
    m_driverController.a().onTrue(new InstantCommand(()->currentState = State.L1).andThen(new MoveToScoringPosition(1, m_wrist, m_elevator)));
    ...
    m_driverController.povLeft().whileTrue(new InstantCommand(() -> currentState = State.AlgaeRemove).andThen(new RunCommand(() -> new AlgaeRemoval(m_coralHand, m_wrist, m_elevator, m_robotDrive, m_funnel).schedule(), m_coralHand, m_wrist, m_elevator, m_robotDrive, m_funnel))).onFalse(new ParallelCommandGroup(new InstantCommand(m_robotDrive::stop, m_robotDrive), new MoveToIntakePositions(m_wrist, m_elevator, m_funnel, m_coralHand)));
    ...
    m_driverController.rightTrigger().whileTrue(new RunCommand(()->m_coralHand.outtake(()->currentState.level), m_coralHand));
    m_driverController.povDown().onTrue(currentState == State.ClimbStart ? new FinishClimb(m_climber).alongWith(new InstantCommand(()->currentState = State.ClimbEnd)) : new ClimbAngles(m_elevator, m_wrist, m_climber, m_funnel).alongWith(new InstantCommand(()->currentState = State.ClimbStart)));
    ...
    m_operatorController.povUp().whileTrue(new RunCommand(()->m_climber.climbCCW(), m_climber)).onFalse(new RunCommand(()->m_climber.stop(), m_climber));
    m_operatorController.povDown().whileTrue(new RunCommand(()->m_climber.climbCW(), m_climber)).onFalse(new RunCommand(()->m_climber.stop(), m_climber));
  }
    ...
    for (int i = 0; i < commands.size(); i ++) {
      ...
      if (fullCommand instanceof Parser.PutCoralCommand) {
        Parser.PutCoralCommand putCmd = (Parser.PutCoralCommand) fullCommand;
          ...
          Command pathCommand = m_robotDrive.AlignToTagFar(reefTags[putCmd.getSide() - 1]).andThen(new WaitCommand(0.8).alongWith(new RunCommand(()->m_robotDrive.stop(), m_robotDrive).withTimeout(0.2)));
          ...
          Command fullSequence = new SequentialCommandGroup(
              new ParallelCommandGroup(pathCommand, new SequentialCommandGroup(
                new SequentialCommandGroup(
                  ...
                  new RunCommand(() -> m_coralHand.intake(), m_coralHand).withTimeout(0.5)), subsystemMovement)),
              new ScoreCoral(
                  putCmd.getLevel(),
                  putCmd.getLeft(),
                  m_coralHand,
          ...
          Command pathCommand = m_robotDrive.PathToPose(PositionCalculations.translateCoordinates(stationTagPose, stationTagPose.getRotation().getDegrees(), 0.3), 0.0).andThen(PositionPIDCommand.generateCommand(m_robotDrive, stationTagPose, Seconds.of(2)));
            ...
            Command intakeSequence = new SequentialCommandGroup(
                new MoveToIntakePositions(m_wrist, m_elevator, m_funnel, m_coralHand),
                new RunCommand(() -> m_coralHand.intake(), m_coralHand).until(()->m_coralHand.hasCoral()).withTimeout(8)
            );
